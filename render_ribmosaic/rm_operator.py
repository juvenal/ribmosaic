...
class RibmosaicOperator():
    ...
    def _dialog_width(self, message, minimum=50):
        """Returns a pixel width large enough to encompass longest line in message
        at current fonts (this is not always the line with most characters
        depending on font and characters used!).
...
class WM_OT_ribmosaic_text_escape(rm_context.ExportContext,
                                  RibmosaicOperator,
                                  bpy.types.Operator):
    """Convert XML control characters in selected text to XML escape sequences"""
...
class WM_OT_ribmosaic_text_comment(rm_context.ExportContext,
    ...
    def execute(self, context):
        try:
            ...
            comment = rm.pipeline_manager.get_element_info(path, attr, "comment")
...
class WM_OT_ribmosaic_panel_duplicate(rm_context.ExportContext,
    ...
    def execute(self, context):
        ...
        try:
            ...
            attr = rm.PropertyHash(p['xmlpath'].replace("/", "") + name + "enabled")
...
class WM_OT_ribmosaic_library_set(rm_context.ExportContext,
    ...
    compile = bpy.props.BoolProperty(name="Compile shaders",
                    description="Compile all shader sources when building library",
                    default=True)
    build = bpy.props.BoolProperty(name="Build slmeta",
                    description="Create slmeta for all shaders when building library",
                    default=True)
    relativepath = bpy.props.BoolProperty(name="Use relative path",
    ...
    def invoke(self, context, event):
        ...
        try:
            if not bpy.data.is_dirty:
                ...
                compile = rm.pipeline_manager.get_attr(self, p, "compile", False)
...
class WM_OT_ribmosaic_library_addpanel(rm_context.ExportContext,
                                       RibmosaicOperator,
                                       bpy.types.Operator):
    """Add shader panels from pipeline's shader library (generated from slmeta)"""
    ...
    autoprops = bpy.props.BoolProperty(name="Auto Assign Properties",
                    description="Automatically assign shader panel to properties"
                                " window based on shader type",
                    default=True)
        ...
        try:
            ...
            if self.library == self.directory:
                ...
                library_path = rm.RibPath(os.path.relpath(self.directory, \
                                       rm.export_manager.export_directory) + os.sep)
            ...
            for filename in self.files:
                if os.path.splitext(filename.name)[1].lower() != ".slmeta":
                    raise rm_error.RibmosaicError(filename.name + " not a .slmeta file")
...
class WM_OT_ribmosaic_pipeline_update(rm_context.ExportContext,
                                       RibmosaicOperator,
                                       bpy.types.Operator):
    """Update ALL shader panels from sources in shader library (updates using slmeta)"""
    ...
    def execute(self, context):
        ...
        try:
            for panel in rm.pipeline_manager.list_elements(category):
                ...
                if slmeta:
                    rm.pipeline_manager.slmeta_to_panel(slmeta, "", self.pipeline,
                                                        "", False)
            rm.pipeline_manager._write_xml(self.pipeline)
            self._refresh_panels()
        except rm_error.RibmosaicError as err:
...
class WM_OT_ribmosaic_pipeline_search(rm_context.ExportContext,
    ...
    def _select_element(self, textdata, xmlpath):
        ...
        try:
            ...
            raise rm_error.RibmosaicError("XML path \"" + xmlpath + "\" not found!")
...
class RENDER_OT_ribmosaic_pass_add(rm_context.ExportContext,
    ...
    def execute(self, context):
        ...
        ribmosaic_passes.collection.move(len(ribmosaic_passes.collection) - 1, 0)
...
class RENDER_OT_ribmosaic_pass_seq_add(rm_context.ExportContext,
                                       RibmosaicOperator,
                                       bpy.types.Operator):
    """Use sequence setup of active pass to generate a pass per frame sequence"""
...
class RENDER_OT_ribmosaic_pass_sort(rm_context.ExportContext,
    ...
    def execute(self, context):
        ...
        for i in list(range(len_passes, -1, -1)) + [-1]:
            ...
            # Find pass with higher or matching group that's not the active pass
